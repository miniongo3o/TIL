...
def replaceData(numData): # numData	2차원 정수 배열
    retData = [ [None for i in numData[0]] for j in numData  ] # 조건에 따라서 전처리된 2차원 배열
...
def getMaxSum(numData): # 요구 사항에 맞춰 처리된 2차원 정수 배열
    ...
    for i in range(len(numData)-1):
        for j in range(len(numData[0])-1):
            temp_sum=numData[i][j] + numData[i][j + 1] + numData[i + 1][j] + numData[i + 1][j + 1]
            if  temp_sum> maxSum:
                maxSum=temp_sum
